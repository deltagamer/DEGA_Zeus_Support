...
if (_activated) then {
	...
	_plane disableCollisionWith _logic;
	...
	_fireNull = true;
	_time = time;
	_offset = if ((typeOf _plane) in Supported_ALL) then {0} else {0};  //Defo Changes height 160
	waituntil {
		...
		_plane setVelocityTransformation [
			_planePos, [_pos select 0,_pos select 1,(_pos select 2) + _offset + _fireProgress * 0], //changes height
			_velocity, _velocity,
		...
		//--- Fire!
		if ((getposasl _plane) distance _pos < 300 && _fireNull) then 
		{
			//--- Create laser target
	...
	sleep 2;
    _plane setVelocity [(vectorDir _plane select 0)*0,(vectorDir _plane select 1)*0,(vectorDir _plane select 2)*12];
	_plane setPos [(getPos _logic) select 0,(getPos _logic) select 1, 22];
	//_plane flyinheight _alt;
    _plane flyInHeight 18;
	[_plane] call ace_fastroping_fnc_equipFRIES;
	...
	private ["_para"];
	{
		...
		//_x setPosATL [(getPosATL _plane select 0) - 8, (getPosATL _plane select 1) + 15, (getPosATL _plane select 2)];	
		_x allowFleeing 0;
	} foreach _type_spawn;		
	
	{ _x addCuratorEditableObjects [_type_spawn,true] } forEach (allCurators);

    [_plane] call ace_fastroping_fnc_deployAI;
	...
	if !(isnull _logic) then 
	{
		...
		waituntil {_plane distance _pos > 2000 || !alive _plane};
	};
